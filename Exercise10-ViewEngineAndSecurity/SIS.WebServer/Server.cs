...
namespace SIS.WebServer
{
	...
	private void TryOpenBrowser(string url)
	{
		...
		{
		    processInfo.FileName = Constants.WindowsCommandPrompt;
		    processInfo.Arguments = string.Format(
			Constants.WindowsStartCommand, url.Replace("&", "^&"));
		}
		if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
		{
		    processInfo.FileName = Constants.LinuxOpenCommand;
		    processInfo.Arguments = url;
		}

		if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
		{
		    processInfo.FileName = Constants.MacOSOpenCommand;
		    processInfo.Arguments = url;
		}
